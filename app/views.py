...
from .models import Toko, Sejarah, Berita, Produk
from .forms import *
...
# labels = ['beras kutah', 'gajah oling', 'gedegan', 'kopi pecah', 'moto pitik', 'paras gempal','sisikan']
...
# model = torch.load('./models/gobatik_model.h5')
...
def scan(request):
    if request.method == 'POST':
        ...
        fs = FileSystemStorage(location='core/static/images/'+datetime.today().date().isoformat())
        filePathName = fs.save(fileObj.name, fileObj)
        filePathName = fs.url(datetime.today().date().isoformat()+'/'+filePathName)
...
def produk_batik(request):
 return render(request, 'frontend/produk-batik.html')
...
def admin_berita(request):
    if request.method == 'POST':
        ...
        fs = FileSystemStorage(location='core/static/berita/thumbnail/'+datetime.now().date().isoformat())
        filePathName = fs.save(fileObj.name, fileObj)
        filePathName = fs.url(datetime.now().date().isoformat()+'/'+filePathName)
        testimage='/static/berita/thumbnail/'+filePathName
...
def admin_toko(request):
    if request.method == 'POST':
        fileObj = request.FILES['thumbnail']
        fs = FileSystemStorage(location='D:/gotik-web-main/gotik-web-main/asetfoto/toko/thumbnail'+datetime.now().date().isoformat())
        filePathName = fs.save(fileObj.name, fileObj)
        filePathName = fs.url(datetime.now().date().isoformat()+'/'+filePathName)
        testimage='D:/gotik-web-main/gotik-web-main/asetfoto/toko/thumbnail/'+filePathName
...
#Admin produk
@login_required(login_url="/login/")
def admin_produk(request):
 return render(request, 'admin/produk/index.html')
